...
directorio_dataset = 'data/Dataset.csv'
ventana=40
clases=7

clases_str = config.actividades.keys()
normalize = False
...
print("Actividades:", datos["A"].unique())
print("Actividades totales de las muestras:", len(datos["A"]))
print(datos["A"][0], datos["A"][300])
plt.hist(datos["A"])
...
model.save_weights('./models/%s_model.h5'%(datetime.datetime.now().strftime("%Y%m%d_%H%M%S")))
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.xticks(range(clases), clases_str)
    plt.yticks(range(clases), clases_str)
...
plt.figure()
plot_confusion_matrix(cm = confusion_mtx, classes = range(clases)) 
plt.show()